...
setTimeout(function () {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (var_setted) {
      const x = e.clientX;
      const y = e.clientY;
      const delta_x_left = X_left - x;
      const delta_x_right = X_right - x;
      const delta_y_left = Y_left - y;
      const delta_y_right = Y_right - y;
      ...
      const adjacent_left = Math.abs(x - X_left);
      let angle_left = Math.acos(adjacent_left / distance_left);
      const adjacent_right = Math.abs(x - X_right);
